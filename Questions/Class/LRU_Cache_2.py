...
class LRUCache:
    ...
    def get(self, key):
        if key not in self.memo:                # 1. Check if in memo, if not return
            return -1
        node = self.memo[key]
    ...
    def put(self, key, value):
        if key in self.memo:                    # 1. Check if key in cache, if yes, update value and call get
            self.memo[key].val = value
            self.get(key)
            return
        if len(self.memo) == self.capacity:
            self.memo.pop(self.head.key)        # 2. If at capacity, remove head key from cache (not the new key)
            self.remove_head_node()             #   Remove head node (oldest)
